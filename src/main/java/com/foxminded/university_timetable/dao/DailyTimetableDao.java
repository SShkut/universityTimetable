...
public class DailyTimetableDao {
	...
	public Optional<DailyTimetable> findDailyTimetableForStudent(Student student, LocalDate date) {
		try {
			DailyTimetable dailyTimetable = this.jdbcTemplate.queryForObject(FIND_DAILY_TIMETABLE_FOR_STUDENT, new Object[] {date, student.getId()}, dailyTimetableRowMapper);
			return Optional.of(dailyTimetable);
		} catch(EmptyResultDataAccessException e) {
			return Optional.empty();
		}
	...
	public Optional<DailyTimetable> findDailyTimetableForTeacher(Teacher teacher, LocalDate date) {
		try {
			DailyTimetable dailyTimetable = this.jdbcTemplate.queryForObject(FIND_DAILY_TIMETABLE_FOR_TEACHER, new Object[] {date, teacher.getId()}, dailyTimetableRowMapper);
			return Optional.of(dailyTimetable);
		} catch(EmptyResultDataAccessException e) {
			return Optional.empty();
		}
	...
	public List<DailyTimetable> findMonthlyTimetableForStudent(Student student, Month month, int year) {
		return this.jdbcTemplate.query(FIND_MONTHLY_TIMETABLE_FOR_STUDENT, new Object[] {YearMonth.of(year, month).atDay(1), YearMonth.of(year, month).atDay(YearMonth.of(year, month).lengthOfMonth()), student.getId()}, dailyTimetableRowMapper);
	}
	
	public List<DailyTimetable> findMonthlyTimetableForTeacher(Teacher teacher, Month month, int year) {
		return this.jdbcTemplate.query(FIND_MONTHLY_TIMETABLE_FOR_TEACHER, new Object[] {YearMonth.of(year, month).atDay(1), YearMonth.of(year, month).atDay(YearMonth.of(year, month).lengthOfMonth()), teacher.getId()}, dailyTimetableRowMapper);
	}
}
